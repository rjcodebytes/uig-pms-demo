...
<div class="pagetitle">
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="{{ url('')}}">Track Procurement</a></li>
        </ol>
    </nav>
</div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <p><strong>Status:</strong>
                                @php
                                    $latestWorkflow = $workflowData->last(); // Get the latest workflow entry
                                @endphp

                                @if ($latestWorkflow)
                                    @if ($latestWorkflow['status'] == 'Pending')
                                        {{$latestWorkflow['status']}} at {{$latestWorkflow['position']}}
                                    @elseif ($latestWorkflow['status'] == 'Approved')
                                        {{$latestWorkflow['status']}} by {{$latestWorkflow['position']}}
                                    @elseif ($latestWorkflow['status'] == 'Rejected')
                                        {{$latestWorkflow['status']}} by {{$latestWorkflow['position']}}
                                    @endif
                                @else
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><a type="button"
                                            href="{{ route('document.view', ['id' => $document->doc_id]) }}"
                                            target="_blank" class="btn btn-primary btn-md mt-2"><i
                                                class="ri-file-text-fill"></i> Uploaded Document</a></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
